...
(function() {
  var API, Serializer, append_slash, callable, debug, hasInsensitive, merge, normalizeXhrResponse, querystring, ref, request,
  ...
  ref = require('./utils'), callable = ref.callable, append_slash = ref.append_slash, merge = ref.merge, hasInsensitive = ref.hasInsensitive, normalizeXhrResponse = ref.normalizeXhrResponse;
  ...
  API = callable((function() {
    ...
    _Class.prototype._xhr = function(request_options, fn) {
      var formData, key, ref1, ref2, ref3, value, white_listed_headers, xhr;
      xhr = new XMLHttpRequest();
      xhr.onload = function(event) {
        var normalizedResponse;
        normalizedResponse = normalizeXhrResponse(xhr);
        return fn(null, normalizedResponse, normalizedResponse.body);
      };
      xhr.open(request_options.method, request_options.url, true);
      white_listed_headers = ['authorization', 'accept'];
      ref1 = request_options.headers;
      for (key in ref1) {
        value = ref1[key];
        if (ref2 = key.toLowerCase(), indexOf.call(white_listed_headers, ref2) >= 0) {
          xhr.setRequestHeader(key, value);
        }
      }
      formData = null;
      if (request_options.formData != null) {
        formData = new FormData();
        ref3 = request_options.formData;
        for (key in ref3) {
          value = ref3[key];
    ...
    _Class.prototype._request = function(method, kwargs, fn) {
      ...
      if (this.opts.http_client === 'xhr') {
        return this._xhr(request_options, fn);
      } else {
